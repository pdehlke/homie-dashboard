...
**Architecture:** Define release token `20260807.5` in the HTML, dynamically load config and helper scripts with that token, and update the supported Lovelace iframe strategy to the same token. Make chart failures visible.
...
**Interfaces:**
- Produces: `HOMIE_ASSET_VERSION = "20260807.5"` and versioned `config.js`/`homie-custom.js` loads.
...
**Interfaces:**
- Produces: iframe URL `/local/community/homie-dashboard/homie-dashboard.html?v=20260807.5`.
